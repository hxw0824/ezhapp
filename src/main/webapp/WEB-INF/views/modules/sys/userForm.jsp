...
<head>
	...
	<script type="text/javascript">
			...
			$("#inputForm").validate({
				...
				submitHandler: function(form){
					...
					if(roleList == "3"){
						...
						var _id = $("#id").val();
						console.log(_id);
						if(_id != '' || !checkOfficeAdmin(oid)){
							loading('正在提交，请稍等...');
							form.submit();
						}else{
							$("#hasKind").css("display","inline-block");
						}
					}else{
						loading('正在提交，请稍等...');
						form.submit();
		...
		function checkOfficeAdmin(oid){
			var _res = true;
			$.ajax({
				url : "${ctx}/sys/user/checkOfficeAdmin",
				type : "POST",
				data :{officeId : oid},
				async : false,
				dataType : "json",
...
<body>
	...
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<div class="control-group" <c:if test="${user.userType ne fns:getDictValue('parent','sys_user_type','')}">style="display:none;"</c:if>>
			<label class="control-label">归属班级:</label>
			<div class="controls">
				<sys:treeselect id="classId" name="classId" value="${user.classId}" labelName="user.classId" labelValue="${user.className}"
					title="班级" url="/sys/cclass/treeData" allowClear="true" isAll="1"/>
				<span class="help-inline"> 调换班级</span>
			</div>
		</div>
		<div class="control-group hidden">
			<label class="control-label">工号:</label>
			<div class="controls">
				<form:input path="no" htmlEscape="false" maxlength="50" value="0001"/>
		...
		<div class="controls"><label id="hasKind" class="error" style="display:none;">该学校已拥有园长角色，请勿重复添加</label></div>
